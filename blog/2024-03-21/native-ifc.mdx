...
To add context to this article, we’ve included insights from Ryan Schultz, a US-based architect with 20+ of experience, founder of [OpeningDESIGN](http://openingdesign.com/) and active member of the [OSArch](https://osarch.org/) community.
...
While BlenderBIM has already switched to using IFC-SPF as a native file format instead of .blend, Yorik’s approach is slightly different. Once the module has been merged, FreeCAD will be storing original IFC data in a container inside a regular FreeCAD file.
