...
The next batch of new features is a series of new joint types that are useful in mechanical design: Rack and Pinion, Screw, Gears, and Belt.
...
## SheetMetal workbench now shipped with Ondsel ES
...
We timed ourselves completing the task — and even though we knew exactly what to do (use a symmetry constraint), we confirmed that a basic operation was indeed taking more time and wasn’t obvious. So we implemented a new midpoint constraint. There are multiple uses for it, but for the sake of a closure let’s illustrate it using the exact scenario in the Teaching Tech video:
...
We also reworked the **Fillet** tool and added a basic **Chamfer** tool. Rather than having two Fillet tools, you now have one where ‘Preserve Corner’ is an option in the Tasks panel.
...
Snapping has been introduced for the dimensions, letting you position them in an easier fashion. Snapping is also introduced for views, in particular for section views to align them with their base views.
...
Lastly, you can easily go back to using the combobox list for switching between workbenches. The option is on the _Workbenches / Available Workbenches_ page of the _Preferences_ dialog. You can get there by clicking the **+** button and selecting _Preferences_ at the bottom of the list of workbenches.

## Quick measure

While we are shipping the new upstream measurement tool, in many cases it’s really convenient to have basic measurements readily at hand at any time. So we are shipping a new feature called _Quick Measure_. It sits on the right side of the sidebar and displays context-sensitive measurements: length for edges, radius for circles etc. You can double-click to select the entire measurement and press **Ctrl+C** to copy it to the clipboard.
...
Originally, we also implemented support for measuring volume. However, the design working group is in favor of keeping it for the upcoming _Physical Properties_ panel that would display information like mass, density, center of gravity, center of volume, and inertia. So we removed it, at least for now.
...
In the future, it will be possible to also install preferred 3rd-party addons.
